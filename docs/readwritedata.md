...
<div class="rmdnote">
<p>The <code>read_csv</code> function has a <code>progress</code> option
that defaults to TRUE. This options provides a nice progress meter while
the CSV file is being read. However, if you are using
<code>read_csv</code> in a function, or perhaps embedding it in a loop,
it’s probably best to set <code>progress = FALSE</code>.</p>
</div>
